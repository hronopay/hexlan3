 ...
 * PUBLIC:    const char *, const DB_ADDRINFO *, DB_ADDRINFO **));
 * PUBLIC: #endif
 */
int
...
#define	INADDR_NONE	((u_long)0xffffffff)
	...
	if ((ret = __os_calloc(env, 1, sizeof(DB_ADDRINFO), &answer)) != 0)
		return (ret);
 ...
 *
 * PUBLIC: #if defined(HAVE_REPLICATION_THREADS)
 * PUBLIC: void __os_freeaddrinfo __P((ENV *, DB_ADDRINFO *));
 * PUBLIC: #endif
 */
void
