...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeHexlanAmount"))
        settings.setValue("nAnonymizeHexlanAmount", 1000);
    nAnonymizeHexlanAmount = settings.value("nAnonymizeHexlanAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeHexlanAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeHexlanAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeHexlanAmount:
            return QVariant(nAnonymizeHexlanAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeHexlanAmount:
            nAnonymizeHexlanAmount = value.toInt();
            settings.setValue("nAnonymizeHexlanAmount", nAnonymizeHexlanAmount);
            emit AnonymizeHexlanAmountChanged(nAnonymizeHexlanAmount);
            break;
        default:
            break;
        }
