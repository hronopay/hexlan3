...
void OverviewPage::setBalance(const CAmount& balance, const CAmount& stake, const CAmount& unconfirmedBalance, const CAmount& immatureBalance, const CAmount& anonymizedBalance, const CAmount& watchOnlyBalance, const CAmount& watchOnlyStake, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    ui->labelWatchAvailable->setText(BitcoinUnits::formatWithUnit(nDisplayUnit, watchOnlyBalance));
    ui->labelWatchStake->setText(BitcoinUnits::formatWithUnit(nDisplayUnit, watchOnlyStake));
    ui->labelWatchPending->setText(BitcoinUnits::formatWithUnit(nDisplayUnit, watchUnconfBalance));
...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeHexlanAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeHexlanAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeHexlanAmount = strAnonymizeHexlanAmount.remove(strAnonymizeHexlanAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeHexlanAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeHexlanAmount*COIN) nMaxToAnonymize = nAnonymizeHexlanAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeHexlanAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeHexlanAmount));
        strAnonymizeHexlanAmount = strAnonymizeHexlanAmount.remove(strAnonymizeHexlanAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeHexlanAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
