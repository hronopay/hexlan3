...
{
    // Convert Hexlan:// to Hexlan:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("Hexlan://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "Hexlan:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Hexlan.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Hexlan.lnk
...
{
    return GetAutostartDir() / "Hexlan.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a Hexlan.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Hexlan\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the Hexlan app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add Hexlan app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Hexlan-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  Hexlan-qt [" + tr("command-line options") + "]                     " + "\n";
