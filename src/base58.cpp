...
namespace {
    ...
    private:
        CHexlanAddress *addr;
    public:
        CHexlanAddressVisitor(CHexlanAddress *addrIn) : addr(addrIn) { }
...
bool CHexlanAddress::Set(const CKeyID &id) {
    SetData(Params().Base58Prefix(CChainParams::PUBKEY_ADDRESS), &id, 20);
    return true;
}

bool CHexlanAddress::Set(const CScriptID &id) {
    SetData(Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS), &id, 20);
    return true;
}

bool CHexlanAddress::Set(const CTxDestination &dest) {
    return boost::apply_visitor(CHexlanAddressVisitor(this), dest);
}
...
bool CHexlanAddress::GetKeyID(CKeyID &keyID) const {
...
bool CHexlanAddress::IsScript() const {
    return IsValid() && vchVersion == Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS);
}

void CHexlanSecret::SetKey(const CKey& vchSecret) {
...
CKey CHexlanSecret::GetKey() {
    CKey ret;
    ret.Set(&vchData[0], &vchData[32], vchData.size() > 32 && vchData[32] == 1);
    return ret;
}

bool CHexlanSecret::IsValid() const {
    bool fExpectedFormat = vchData.size() == 32 || (vchData.size() == 33 && vchData[32] == 1);
    bool fCorrectVersion = vchVersion == Params().Base58Prefix(CChainParams::SECRET_KEY);
    return fExpectedFormat && fCorrectVersion;
}

bool CHexlanSecret::SetString(const char* pszSecret) {
    return CBase58Data::SetString(pszSecret) && IsValid();
}

bool CHexlanSecret::SetString(const std::string& strSecret) {
