...
# Отключаем предупреждения, чтобы видеть только ошибки
QMAKE_CXXFLAGS += -w
...
# !!! ИСПРАВЛЕННЫЙ ПУТЬ !!!
# Папка deps_sources лежит ВНУТРИ текущей папки
...
# Стандартные системные библиотеки Windows
...
# Используем $$PWD для абсолютных путей, чтобы qmake не терялся
